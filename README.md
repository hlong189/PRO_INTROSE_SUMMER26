# NT-II — Website bán mỹ phẩm Fullstack
...
Nghĩa là MySQL trên máy bạn có đặt mật khẩu cho root, nhưng file `.env` đang để trống mật khẩu. 

**Mở file `.env`** ở thư mục gốc và điền đúng mật khẩu MySQL của bạn vào `DB_PASSWORD`, ví dụ:
...
Nghĩa là database đang trống (mới chạy `init-db` chứ chưa `seed/import-dataset`, hoặc đã chạy nhưng bị lỗi giữa đường). 

Chạy lệnh sau để xóa sạch và tạo lại từ đầu trong 1 lệnh duy nhất:
...
Project đã kèm 13 file CSV thật trong `server/db/dataset/` (50 sản phẩm, 50 khách hàng, 50 đơn hàng, 128 mục đơn hàng, đánh giá, lô hàng, voucher...). 

Script `import-dataset.js` sẽ:
1. Xóa sạch dữ liệu cũ trong 12 bảng (TRUNCATE theo đúng thứ tự an toàn)
2. Nhập lại từ CSV theo đúng thứ tự khóa ngoại: `roles` → `users` → `suppliers` → `products` → `inventory_batches` → `vouchers` → `orders` → `order_items` → `payments` → `shippings` → `skin_profiles` → `reviews`
...
Đăng nhập admin → **Quản lý kho** → tab **Sản phẩm**
- Bấm **"⇪ Nhập CSV"** → chọn file → Nhập dữ liệu
...
### M1 - Người dùng
- Mã khôi phục rút về đúng 6 số (hết hạn 10 phút)
- Validate họ tên (chỉ chữ)
- SĐT bắt buộc đúng 10 số
- Mật khẩu bắt buộc ≥8 ký tự gồm chữ/số/ký tự đặc biệt
- Validate SĐT/Tỉnh-Thành/Phường-Xã khi cập nhật hồ sơ
- Đổi mật khẩu chặn đúng khi sai mật khẩu cũ + bắt buộc mật khẩu mới khác mật khẩu cũ
- Thêm khóa/mở khóa tài khoản vi phạm (tab Khách hàng)

### M2 - Sản phẩm
- SKU tự gợi ý theo danh mục (SKU-DANHMUC-0001)
- Danh mục/Nhãn hàng chọn từ danh sách có sẵn (datalist)
- Validate giá bán/giá gốc/tồn kho không âm
- Modal xác nhận khi xóa sản phẩm
- Thêm nút "Hiện lại" cho sản phẩm đã ẩn
- Bỏ số lượng trong dropdown lọc
- Thêm tab Khuyến mãi (tạo voucher theo chiến dịch)

### M3 - Giỏ hàng & Đơn hàng
- Giỏ hàng tách riêng theo từng tài khoản (sửa lỗi nghiêm trọng liên thông giữa các account)
- Modal xác nhận khi giảm số lượng về 0
- Sửa lỗi nhập số lượng >9 bị cắt số
- Mã giảm giá chọn từ dropdown thay vì nhập tay
- Validate Tỉnh-Thành/Phường-Xã không được chỉ là số
- VNPay/Momo/Chuyển khoản hiện mã QR/thông tin thanh toán + bắt xác nhận trước khi hoàn tất đơn
- Modal xác nhận khi hủy đơn

### M4 - Kho & Báo cáo
- Validate số lượng/giá nhập kho không âm
- Mã lô hàng bắt buộc duy nhất (báo lỗi nếu trùng)
- Thêm mục "Đề xuất hàng cần nhập" theo tồn kho thấp + nhà cung cấp gần nhất

### M5 - Đánh giá
- Chỉ cho đánh giá nếu đã mua và đơn ở trạng thái Hoàn tất
- Bắt buộc chọn sao VÀ nhập nội dung (không còn mặc định 5 sao)
- Thêm nút "Đánh giá" ngay trong Lịch sử mua hàng cho đơn đã hoàn tất
- Admin có thể phản hồi đánh giá khách hàng (tab Đánh giá)

### M6 - Vận chuyển
- Trạng thái vận chuyển tự đồng bộ theo trạng thái đơn hàng
- Khi admin chuyển đơn sang Hủy/Trả hàng, tồn kho được hoàn lại tự động
- Làm rõ vai trò mã vận đơn (hiển thị ghi chú "dùng để tra cứu với đơn vị vận chuyển")

### M7 - Cá nhân hóa
- Khám phá làn da: vào lại trang sẽ hiện đúng kết quả đã lưu trước đó (không bị mất)

## ⚠️ Chưa thể khắc phục / cần thêm thông tin

### FR-M7-CN-04 đến FR-M7-CN-11
Báo cáo không nêu rõ nội dung các yêu cầu này — vui lòng cung cấp tài liệu đặc tả chi tiết để mình triển khai đúng.

### Cập nhật ảnh đại diện
Bảng `users` trong `SE_NTII.sql` không có cột lưu ảnh — cần bổ sung cột (vd. `avatar_url`) nếu muốn có tính năng này.

### Ghi nhận công nợ nhà cung cấp
Schema hiện chưa có bảng theo dõi công nợ/thanh toán cho nhà cung cấp — cần thiết kế thêm bảng (vd. `supplier_payables`) nếu muốn quản lý đầy đủ.

## ⚠️ Lưu ý khác

1. **"Quên mật khẩu"** trả mã khôi phục trực tiếp trên UI vì chưa tích hợp dịch vụ gửi email thật — khi triển khai thật, hãy gửi mã này qua email thay vì hiển thị.

2. **Phương thức thanh toán VNPay/Momo/Bank_Transfer** hiện là mô phỏng (chưa gọi API cổng thanh toán thật), nhưng khách phải xác nhận đã thanh toán trước khi đơn được tạo, và trạng thái được lưu đúng vào bảng `payments`.

3. **Muốn khởi tạo lại database từ đầu:** `npm run reset-db` hoặc `npm run reset-db -- --real`.
